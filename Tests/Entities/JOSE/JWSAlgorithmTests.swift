...
class JWSAlgorithmTests: XCTestCase {
  ...
  func testAlgorithmTypeNameAndRequirement() {
    for type in JWSAlgorithm.AlgorithmType.allCases {
      ...
      switch type {
      case .HS256:
        XCTAssertEqual(type.requirement, .REQUIRED)
      case .HS384, .HS512, .RS384, .RS512, .ES256K, .ES384, .ES512, .PS256, .PS384, .PS512, .EdDSA:
        XCTAssertEqual(type.requirement, .OPTIONAL)
      case .RS256, .ES256:
        XCTAssertEqual(type.requirement, .RECOMMENDED)
      }
    }
  }
}
